...
int dam_plugin::create_device(deviceblock *db)
{
  ...
  if ( strcasecmp(db->argv0,"device_dam") == 0 ) 
	...
	{
	  int trigger = get_value ( db->argv3);
	  int self_trigger = get_value ( db->argv4);
  ...
  if ( strcasecmp(db->argv0,"device_tpcfee") == 0 ) 
    {
      // we need at least 2 params
      if ( db->npar <3 ) return 1; // indicate wrong params
      ...
      if ( db->npar == 3)
	{
	  ...
	  add_readoutdevice ( new daq_device_tpcfee( eventtype,
						  subid ));
	  return 0;  // say "we handled this request" 
	}
...
void dam_plugin::identify(std::ostream& os, const int flag) const
{
  ...
  if ( flag <=2 )
  ...
  else
    {
      ...
      os << " -     device_dam (evttype, subid [, trigger, self_trigger] ) - DAM FELIX Board " << std::endl;
      os << " -     device_tpcfee (evttype, subid [, trigger] ) - TPC 2-SAMPA Board " << std::endl;
    }
